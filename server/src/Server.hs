...
mainServer :: Config -> Server MainAPI
mainServer cfg = convertServer cfg $
         listProjects
    :<|> render
    :<|> update
    :<|> getSource

staticServer :: Config -> Server StaticAPI
staticServer cfg name file = liftIO $ B.readFile (configDataDir cfg </> name </> file)

server :: Config -> Server API
server cfg = mainServer cfg
     :<|> staticServer cfg
